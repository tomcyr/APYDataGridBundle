    ...
    */
    ...
    public function getGridCell($column, $row, $grid)
    {
        ...
        foreach ($values as $sourceValue) {
            ...
            {
                $return[] = $this->renderBlock($block, array('grid' => $grid, 'column' => $column, 'row' => $row, 'value' => $value, 'sourceValue' => $sourceValue));
            } else {
                $return[] = $this->renderBlock('grid_column_cell', array('grid' => $grid, 'column' => $column, 'row' => $row, 'value' => $value, 'sourceValue' => $sourceValue));
                $block = null;
            }
